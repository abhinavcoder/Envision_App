...
def query_depth(input_dir, output, snaps):
	...
	for snapshot in os.listdir(snaps):
		print ("loading coffee")
		if not snapshot.endswith("caffemodel"):
			continue
		currentSnapDir = snapshot.replace(".caffemodel","")
		if os.path.exists(output + "_abs/" + currentSnapDir):
			shutil.rmtree(output + "_abs/" + currentSnapDir)
		os.mkdir(output + "_abs/" + currentSnapDir)
		print(currentSnapDir)
		sys.stdout.flush()
		netFile = snapshot.replace(".caffemodel",".prototxt")
		net = caffe.Net(snaps + '/' + netFile, snaps + '/' + snapshot, caffe.TEST)
		...
		for count, file in enumerate(os.listdir(input_dir)):
			...
			filename = 'tmp'#os.path.splitext(os.path.basename(inputFileName))[0]
			filePathAbs = 'output_abs/tmp.png'
